...
**执行流程**：
1. **对比分析 (Diff Analysis)**：自动对比 `content/drafts/` 中的 AI 原稿与用户的修改稿。
2. **偏好提取**：
   - **词汇偏好**：用户删掉了哪些“AI 常用词”？替换成了哪些“个人高频词”？
   - **语气调整**：是变得更辛辣、更温润还是更极简了？
   - **句式偏好**：是否将长句拆成了短句？是否调整了逻辑顺序？
3. **展示学习报告**：AI 总结本次学习到的 3 个核心偏好，并向用户确认。
4. **写回档案**：更新 `system/profile.json` 中的 `learned_from_edits` 字段。
